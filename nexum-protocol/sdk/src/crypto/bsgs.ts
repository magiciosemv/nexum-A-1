 ...
 */
export function buildBSGSTable(onProgress?: (pct: number) => void): BSGSTable {
  const table = new Map<bigint, number>();
  const yTable = new Map<bigint, bigint>();
...
function affineAddLaw1(
  ...
  if (numX === 0n) return null;

  const denomX = modP(1n + k);
  const denomY = modP(1n - k);

  if (denomX === 0n || denomY === 0n) return null;
...
function modPInverse(a: bigint): bigint {
  const aMod = modP(a);
  if (aMod === 0n) return 0n;
  let old_r = aMod, r = P;
  let old_s = 1n, s = 0n;
  while (r !== 0n) {
    const q = old_r / r;
 ...
 */
export function bsgsDecrypt(ct: Ciphertext, sk: bigint, bsgsTable: BSGSTable): bigint {
...
export function decryptBalance(
  ct_lo_bytes: Uint8Array,
  ct_hi_bytes: Uint8Array,
  sk: bigint,
  table: BSGSTable,
  deserialize: (buf: Uint8Array) => Ciphertext,
): bigint {
