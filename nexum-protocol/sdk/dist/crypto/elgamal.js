...
// ── ElGamal encrypt ─────────────────────────────────────────────────────────
export function encrypt(m, pk, r) {
    if (m < 0n || m >= (1n << 32n)) {
...
export function serializeCiphertext(ct) {
...
export function deserializeCiphertext(buf) {
    if (buf.length !== 128)
...
export async function deriveKeyPair(signFunction) {
...
function writeBigInt32LE(buf, n, offset) {
    for (let i = 0; i < 32; i++) {
        buf[offset + i] = Number((n >> BigInt(i * 8)) & 0xffn);
    }
}
function readBigInt32LE(buf, offset) {
    let n = 0n;
    for (let i = 0; i < 32; i++) {
...
async function hkdf(ikm, info, len) {
    ...
    const key = await crypto.subtle.importKey("raw", ikm.buffer, { name: "HKDF" }, false, ["deriveBits"]);
    const bits = await crypto.subtle.deriveBits({ name: "HKDF", hash: "SHA-256", salt: salt.buffer, info: info.buffer }, key, len * 8);
